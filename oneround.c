...
#define Nb 4 // block size (AES = 4 words = 16 bytes)

// AES S-box in 16x16 matrix
...
void build_inv_sbox()
{
    for (int row = 0; row < 16; row++)
    {
        for (int col = 0; col < 16; col++)
        {
            ...
            inv_sbox[val >> 4][val & 0x0F] = (row << 4) | col;
        }
    }
}
...
void inv_sub_bytes(uint8_t state[16])
{
    for (int i = 0; i < 16; i++)
    {
        state[i] = inv_sbox[state[i] >> 4][state[i] & 0x0F];
    }
}
...
void add_round_key(uint8_t state[16], uint8_t key[16])
{
    for (int i = 0; i < 16; i++)
    {
        state[i] ^= key[i];
...
void aes_one_round_encrypt(uint8_t state[16], uint8_t key[16])
{
...
void aes_one_round_decrypt(uint8_t state[16], uint8_t key[16])
{
    add_round_key(state, key);
...
int main()
{
    ...
    aes_one_round_encrypt(state, key);
    print_state("Ciphertext", state);

    aes_one_round_decrypt(state, key);
